...
class SpherebotremoteGlade
  ...
  def initialize(path_or_data, root = nil, domain = nil, localedir = nil, flag = GladeXML::FILE)
    ...
    @glade['motorSpeed'].can_focus= false
  end
  ...
  def on_connectButton_clicked(widget)
    serial = @glade['serialPortEntryText'].text
    @xiphos = Xiphos.new(serial)
    puts "Xiphos connected"
  end
  ...
  def on_window_key_press_event(widget, arg0)
   if (@xiphos != nil)
     case( Gdk::Keyval.to_name(arg0.keyval) )
        when "Up"   : puts "increasing speed"
        else puts "#{Gdk::Keyval.to_name(arg0.keyval)} not implemented"
     end
   else 
     puts Gdk::Keyval.to_name(arg0.keyval)
   end
